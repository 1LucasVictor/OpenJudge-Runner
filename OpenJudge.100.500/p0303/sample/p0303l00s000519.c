...
int main(void)
{
		...
		&& (truck_num <= MAX_TRUCK_NUM))
	{
		...
		memset(truck_buff, 0, sizeof(truck_buff));
...
//		printf("\n");
		...
		{
			truck_min = 0;
			for(truck_loop = 0; truck_loop < truck_num; truck_loop++)
			{
				temp = truck_buff[truck_loop] + weight_buff[loop];
				if(temp <= ave)
				{
					truck_buff[truck_loop] += weight_buff[loop];
					
					if(truck_buff[truck_max] < truck_buff[truck_loop])
					{
						truck_max = truck_loop;
					}
					else
					{
						/* nothing */
				...
				{
					if(truck_buff[truck_min] > truck_buff[truck_loop])
					{
						truck_min = truck_loop;
					}
					else
					{
						/* nothing */
			...
			if(truck_loop == truck_num)
			{
				truck_buff[truck_min] += weight_buff[loop];
				
				if(truck_buff[truck_max] < truck_buff[truck_min])
				{
					truck_max = truck_min;
				}
				else
