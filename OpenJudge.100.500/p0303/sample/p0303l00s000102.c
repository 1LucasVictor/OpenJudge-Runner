...
//----------------------------------------------------------------------------
void carshift(TCarSet *car, long max, long min)
{
...
void input(void)
{
    ...
    long lastmax, lastminp, lastmaxp;
    unsigned short *ary;
    TCarSet *carset;
    ...
    if (max!=min){
        for (lastmax=max; max<=lastmax;){
            lastmax=max, lastminp=minp, lastmaxp=maxp;
            carshift(carset, maxp, minp);
        ...
        carshift(carset, lastminp, lastmaxp);
    }
    for (i=max=0; i<K; i++){
        val=sumcar(&carset[i], ary);
#ifdef DESKTOP
...
#ifdef DESKTOP
    printf("min:(%d)%d, max:(%d)", minp, min, maxp);
#endif
    printf("%ld\n", max);
    free(carset);
    free(ary);
