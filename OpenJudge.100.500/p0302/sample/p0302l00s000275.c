...
int main(){
  int val,i,j,n,dic=0,dicc=1<<29,index2=0,vall;
  char op[10],s[12];
  int exist=-1,index=0;

  while(scanf("%d",&n) != EOF){
    dic=0,dicc=1<<29;
    for(i=0;i<=MAX;i++)strcpy(&stack[i][0],"notFound");
	...
	while(1){
	  ...
	  //printf("val = %d",val);
	  //if(stack[val][0] == '\0'){
	  if(strcmp(&stack[val][0],"notFound") == 0){
	    if(val>dic)dic=val;
	...
	while(1){
	  ...
	  else if(strcmp(&stack[vall][0],"notFound") == 0){
	    break;
	  }
	  index2++;
