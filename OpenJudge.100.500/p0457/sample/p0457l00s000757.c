...
int main() {
    ...
    while (a < N || b < M) {
        ll temp;
        if (a == N) temp = B[b++];
        else if (b == M) temp = A[a++];
        else if (A[a] > B[b]) temp = B[b++];
        else if (A[a] < B[b]) temp = A[a++];
        else temp = A[a + 1] > B[b + 1] ? B[b++] : A[a++];
        if (temp > K - sum) {
            if (temp == A[a - 1]) {
                a--;
                if (b < M - 1 && A[a] + A[a - 1] > B[b] + B[b + 1]) {
                    sum -= A[a--];
                    temp = B[b++];
                }
                else break;
