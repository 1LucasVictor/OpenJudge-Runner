...
Node* findMinimum(Node *target) {
    ...
    else                return findMinimum(target->left);
}
...
void delete(Node *target) {
    if (target->parent) {
        ...
        if (target->left == NULL && target->right == NULL) {
            ...
            /// exist children
            Node *minimumNode = findMinimum(target->right);
            swap(target, minimumNode);
            delete(minimumNode);

        } else if (target->left) {
            /// exist left child only
            if (target == target->parent->left) target->parent->left = target->left;
            ...
            if (target == target->parent->left) target->parent->left = target->right;
