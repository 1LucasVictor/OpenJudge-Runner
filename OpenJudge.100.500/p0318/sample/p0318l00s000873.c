...
Node treeSuccessor(Node x){
  Node y;
  if(x->right != NIL) return treeMinimum(x->right); //case1
...
void treeDelete(Node z){
  ...
  if(z->left == NIL || z->right == NIL) a = z;
  else a = treeSuccessor(z);

  if(a->left != NIL) b = a->left;
  else b = a->right;

  if( b != NIL ) b->parent = a->parent;
