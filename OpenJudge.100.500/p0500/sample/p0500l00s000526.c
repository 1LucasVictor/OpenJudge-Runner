...
int main(void) {
	...
	for (i = 0; i < m; i++) {
		...
		if (n == 2 && s == 1) { i_cout(-1); return 0; }

		if (a[s - 1] == (char)c + '0') continue;
		if (a[s - 1] != '*') { i_cout(-1); return 0; }
		a[s - 1] = (char)c + '0';
	}

	
	if (n == 3 && a[0] == '0') { i_cout(-1); return 0; }
	if (n == 2 && a[1] == '0') { i_cout(-1); return 0; }

	for (i = 0; i < 3; i++)
		if (a[i] == '*')a[i] = '0';

	ans = atoi(a);
	if (n == 3 && ans < 100) ans += 100;
	if (n == 2 && ans < 10)  ans += 10;
	if (n == 2 && ans >= 100) ans = -1;
	if (n == 1 && ans >= 10) ans = -1;
